  ...
  return (
    <div className="min-h-screen bg-slate-100 pt-20 text-slate-950 dark:bg-gradient-to-br dark:from-[#1a1f2e] dark:to-[#0f1419] dark:text-white">
      <main className="max-w-7xl px-4 pb-10 md:px-6">
                    ...
                    <div className="space-y-1">
                      {dateEvents.slice(0, 4).map((event) => (
                        <Link
                          ...
                          className={`block truncate rounded-md px-2 py-1 text-xs font-bold leading-5 ${eventTypeStyles[event.event_type]}`}
                        >
        ...
        {isLoading && (
          <div className="mt-4 rounded-lg border border-slate-300 bg-white p-4 text-center text-base font-semibold text-slate-600 dark:border-slate-700 dark:bg-slate-900/80 dark:text-gray-300">
            読み込み中...
          </div>
        )}
      </main>
